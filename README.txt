...
Lab 4: "Write a client and a server program in C/C++ using TCP/IP. 
The server program generates
randomly a rather huge number (close to INT_MAX, see for details limits.h), and the client(s)
should decide if the number is prime or not. 
The checking mechanism should be as follows:
...
##To compile: (Using library linkers and no input file path for default values)
